...
export default function MealForm() {
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      ...
      queryClient.invalidateQueries({ queryKey: [`/api/diets/${dietId}`] }); // to update total calories
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        ...
        <Card className="p-6 rounded-2xl shadow-sm border-border/50">
          <div className="flex justify-between items-center mb-4 border-b pb-2">
            ...
            <Button type="button" variant="outline" size="sm" className="rounded-xl text-primary" onClick={() => append({ name: "", quantity: "", calories: 0, protein: 0, carbs: 0, fat: 0 })}>
          ...
          <div className="space-y-4">
            {fields.map((field, index) => (
              <div key={field.id} className="flex gap-4 items-start p-4 bg-secondary/30 rounded-xl border border-border/50">
                <div className="grid grid-cols-2 md:grid-cols-6 gap-3 flex-1">
                  <div className="col-span-2 space-y-1">
                    <Label className="text-xs">Alimento *</Label>
                    <Input placeholder="Aveia em flocos" className="h-9 rounded-lg" {...register(`foods.${index}.name`)} />
